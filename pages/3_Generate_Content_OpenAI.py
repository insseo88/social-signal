...
def finalize_text_and_flags(text: str, m: dict, brand_name: str, all_keywords: list = None, target_url: str = None):
    ...
    if text:
        ...
        if first_three_words:
            ...
            # 保持列表不超过 20 个，只保留最新的
            if len(st.session_state.used_start_phrases) > 20:
                st.session_state.used_start_phrases = st.session_state.used_start_phrases[-20:]
...
def init_clients():
    supabase_client, openai_ready_flag = None, False
    openai_client = None # 先声明
    ...
    try:
        # 初始化 OpenAI 客户端
        import openai # 确保这里导入了 openai
        openai_client = openai.OpenAI(
            api_key=st.secrets["openai_api_key"]
        )
        openai_ready_flag = True
    except Exception: 
        st.error("❌ 未找到 OpenAI API 密钥。")
        
    return supabase_client, openai_client, openai_ready_flag
...
# $X / 1M tokens = $X / 1,000,000 tokens
# --- 更新模型定价为 GPT-5 系列 ---
MODEL_PRICING = {
    # --- GPT-5 Pricing from image_5d5ff0.png (Per 1 Million tokens, converted to per token) ---
    "gpt-4.1":        {"input": 1.250 / 1000000, "output": 10.000 / 1000000},     # Input: $1.250/1M, Output: $10.000/1M
    "gpt-4.1-mini":     {"input": 0.250 / 1000000, "output": 2.000 / 1000000},      # Input: $0.250/1M, Output: $2.000/1M
    "gpt-5-nano":     {"input": 0.050 / 1000000, "output": 0.400 / 1000000},      # Input: $0.050/1M, Output: $0.400/1M
}
...
def generate_text_from_ai_gpt(openai_client, model: str, max_tokens: int, system_prompt: str, user_prompt: str, temperature: float = 0.9):
    messages = [
    ...
    try:
        # 使用 call_with_retry 包裹 OpenAI 调用
        response = call_with_retry(lambda: openai_client.chat.completions.create(**request_args))
        ...
        st.error(f"❌ OpenAI (GPT) text generation call failed. Error: {e}")
        return None, 0.0
    ...
    if st.button("💡 Brainstorm Diverse Chat Topics"):
        with st.spinner("AI is brainstorming diverse chat topics..."):
            ...
            **Example**: 
            ...
            """
            try:
                # --- 替换模型：使用 gpt-4.1 来保证高准确率的 JSON 输出 ---
                response = call_with_retry(lambda: openai_client.chat.completions.create(
                    model="gpt-4.1", 
                    messages=[
                        {"role": "system", "content": "You are a precise JSON generation bot. You only output valid JSON."},
        ...
        if existing_post_count < total_posts_required:
            ...
            for i in range(existing_post_count, total_posts_required):
                ...
                try:
                    ...
                    # --- 替换模型：使用 gpt-4.1 ---
                    post_text, post_cost = generate_text_from_ai_gpt(openai_client, model="gpt-4.1", system_prompt=system_prompt_for_post, user_prompt="Go.", max_tokens=150, temperature=0.9)
                    ...
                    for j in range(num_comments_per_post):
                        ...
                        system_prompt_for_comment = ULTIMATE_SYSTEM_PROMPT.format(
                            context_block=f'You are replying to this post: "{final_post_text}". {start_exclusion_instruction}',
                        ...
                        # --- 替换模型：使用 gpt-4.1-mini ---
                        comment_text, comment_cost = generate_text_from_ai_gpt(openai_client, model="gpt-4.1-mini", system_prompt=system_prompt_for_comment, user_prompt="Go.", max_tokens=80, temperature=0.95)
                    ...
                    for j in range(num_quotes_per_post):
                        ...
                        elif qt_meta.get("k") and qt_meta["k"] and isinstance(qt_meta["k"], list) and len(qt_meta["k"]) > 0: # ❗ 增加类型检查
                            kw = qt_meta['k'][0]
                            # 如果 Quote 被分配了关键词，使用强约束
                            core_content_instruction_quote = (
                                f"🚨 CRITICAL MENTION: You **MUST** integrate the project keyword '{kw}' naturally into your reply, "
                                f"treating it as a proper noun or specific system name. "
                                f"**Integration Method:** Place it after a verb (e.g., 'I just **saw** the {kw} update')."
                            )
                        else:
                            # 都没有分配，则保持排除 Brand/URL/Keyword 的指令
                        ...
                        system_prompt_for_quote = ULTIMATE_SYSTEM_PROMPT.format(
                            context_block=f'You are quoting this post: "{final_post_text}". {start_exclusion_instruction}',
                            content_type="quote", 
                            language=plan_item['language'], 
                        ...
                        # --- 替换模型：使用 gpt-4.1-mini ---
                        quote_text, quote_cost = generate_text_from_ai_gpt(openai_client, model="gpt-4.1-mini", system_prompt=system_prompt_for_quote, user_prompt="Go.", max_tokens=100, temperature=0.9)
                    ...
                    for j in range(num_quotes_per_post):
                        ...
                        if qt_meta.get("b") == 1 and brand_name:
                            ...
                            core_content_instruction_quote = (
                                f"🚨 CRITICAL MENTION: You **MUST** integrate the project keyword '{kw}' naturally into your reply, "
                                f"treating it as a PROPER NOUN or specific system name. "
                                f"INTEGRATION METHOD (MANDATORY): Place it after a verb (e.g., 'I just saw the {kw} update')." # ❗ 移除双星号
                            )
                        else:
                            # 都没有分配，则保持排除 Brand/URL/Keyword 的指令
                            core_content_instruction_quote = "You MUST NOT include the brand name or URL in your reply. Do NOT mention any specific keywords."

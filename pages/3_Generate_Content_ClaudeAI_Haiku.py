...
import os
import string
import openai # 替换 anthropic 为 openai
...
def finalize_text_and_flags(text: str, m: dict, brand_name: str, all_keywords: list = None, target_url: str = None):
    ...
    # --- 1. 基础清理 & 符号标准化 ---
    text = text.strip()
    # 替换中文/智能引号为标准引号 (Smart Quotes Normalization)
    text = text.replace('“', '"').replace('”', '"').replace("‘", "'").replace("’", "'")

    # --- 2. 暴力移除所有双引号 ---
    text = text.replace('"', '')

    # --- 3. 智能移除作为包裹用途的单引号 ('word' -> word)，保留缩写 (don't) ---
    text = re.sub(r"(^|\s)'", r"\1", text)
    text = re.sub(r"'(?=\s|$|[.,!?;:])", "", text)

    # ❗ 新增：强制移除任何残留的双星号/粗体标记
    text = text.replace('**', '')
    ...
    if target_url:
        ...
        for suffix in common_suffixes:
            # 这里的逻辑有问题，它把后缀换成了完整的 target_url，可能会导致 URL 重复
            # 保持原代码逻辑，但这不是修复的重点
    ...
    # --- 5. 关键词硬性插入逻辑 (新/修改) ---
    keyword_hit_flag = 0 # 默认为 0
    if m.get("k") and m["k"]:
        # 确保 m["k"] 是列表，不是整数，才能访问 [0]
        if isinstance(m["k"], list) and len(m["k"]) > 0:
            keyword = m["k"][0]
            keyword_pattern = re.escape(keyword)
            
            # 检查关键词是否已经存在于文本中（确保它是一个独立的词）
            keyword_regex = re.compile(rf'(?<![0-9A-Za-z]){keyword_pattern}(?![0-9A-Za-z])', re.IGNORECASE)

            # ❗ 替换了原有的逻辑，只检查是否命中，不进行硬塞
            if keyword_regex.search(text):
                keyword_hit_flag = 1
            else:
                # 关键词缺失：不进行硬塞，标记为 0，留待人工检查
                keyword_hit_flag = 0
        
    # ❗ 关键修正：将命中标记放在新的键 'kw_hit' 上，而不是覆盖 m["k"]
    m["kw_hit"] = keyword_hit_flag

    # --- 以下逻辑保持不变 ---
    # ... (Brand/URL 处理逻辑，注意它们仍然使用 m["u"] 和 m["b"])
    ...
    if text.strip().endswith('#'):
        ...
        if target_url:
            domain = re.sub(r'https?://(?:www\.  )?', '', str(target_url)).split('.')[0]
    ...
    has_url = bool(URL_RE.search(text) or (m.get("u_val") and str(m["u_val"]).lower() in text.lower()))
    ...
    if text:
        # 提取前三个词，去除标点，转换为小写
        first_three_words = " ".join(text.split()[:3]).replace(',', '').replace('.', '').replace('!', '').replace('?', '').strip().lower()
        if first_three_words:
            st.session_state.used_start_phrases.append(first_three_words)
            # 保持列表不超过 50 个，只保留最新的
            if len(st.session_state.used_start_phrases) > 50:
                st.session_state.used_start_phrases = st.session_state.used_start_phrases[-50:]

    return text, m # 确保 m["k"] 仍然是一个列表，除非它在前面被覆盖了
...
# --- 客户端初始化：替换 Anthropic 为 OpenAI ---
def init_clients():
    supabase_client, claude_ready_flag = None, False
    claude_client = None # 声明 Anthropic 客户端
    ...
    try:
        # 初始化 Anthropic 客户端
        import anthropic # 确保这里导入了 anthropic
        claude_client = anthropic.Anthropic(
            api_key=st.secrets["anthropic_api_key"] # 假设您的密钥在 st.secrets["anthropic_api_key"]
        )
        claude_ready_flag = True
    except Exception: 
        st.error("❌ 未找到 Anthropic API 密钥。")
        
    return supabase_client, claude_client, claude_ready_flag

# 更改初始化函数调用和变量名 (在 `init_clients` 定义之后)
# 查找: supabase, openai_client, openai_ready = init_clients()
# 替换为:
supabase, claude_client, claude_ready = init_clients() 

if not supabase or not claude_ready:
    st.error("客户端未初始化。")
    st.stop()

# 追踪最近使用的 Post、Comment 和 Quote 开头（防止重复）
if 'used_start_phrases' not in st.session_state:
    # 存储最近 20 个（Post/Quote/Comment 混合）的开头句
    st.session_state.used_start_phrases = []

# 更改初始化函数调用和变量名
supabase, openai_client, openai_ready = init_clients()

if not supabase or not openai_ready:
    st.error("客户端未初始化。")
    st.stop()
...
MODEL_PRICING = {
    # 假设 Post (高品质) 使用 Sonnet 4.5, Comment/Quote (低成本/速度) 使用 Haiku 4.5
...
# --- 模型生成函数：替换 Claude 为 OpenAI ---
def generate_text_from_ai_claude(claude_client, model: str, max_tokens: int, system_prompt: str, user_prompt: str, temperature: float = 0.9):
    
    request_args = {
        "model": model,
        "system": system_prompt, # Claude 3 模型的 system prompt 是一个独立参数
        "messages": [
            {"role": "user", "content": user_prompt}
        ],
        "max_tokens": max_tokens,
        "temperature": temperature,
        "stop_sequences": ["</end>"] # 可选：添加停止序列以提高输出格式的可靠性
    }
    
    try:
        # 使用 call_with_retry 包裹 Anthropic 调用
        response = call_with_retry(lambda: claude_client.messages.create(**request_args))
        
        # Claude 的响应结构不同
        content = response.content[0].text.strip() if response.content and response.content[0].type == 'text' else None
        
        if content is None:
            raise ValueError("Claude response content was empty or not in text format.")
            
        # Claude 的 token 统计结构不同
...
with st.sidebar:
    ...
    if not supabase or not openai_ready: # 检查新的 openai_ready
        st.error("客户端未初始化。")
        st.stop()
    ...
    with st.expander("Comment Settings"):
        ...
        comment_keyword_percentage = st.slider("Keywords %", 0, 100, 30, key="comment_keyword_pct")
    ...
    with st.expander("Quote Settings"):
        ...
        quote_keyword_percentage = st.slider("Keywords %", 0, 100, 30, key="quote_keyword_pct")
...
if topic_mode == "Manual Input":
    ...
    num_topics_to_generate = st.slider("How many diverse topics to generate?", 5, 100, 50)
    ...
    if st.button("💡 Brainstorm Diverse Chat Topics"):
        with st.spinner("AI is brainstorming diverse chat topics..."):
            ...
            **Example**: 
            ...
            """
            try:
                # --- 替换模型：使用 claude-sonnet-4.5 来保证高准确率的 JSON 输出 ---
                response = call_with_retry(lambda: claude_client.messages.create(
                    model="claude-sonnet-4-5-20250929", 
                    messages=[
                        {"role": "user", "content": ai_topics_prompt}
                    ],
                    system="You are a precise JSON generation bot. You only output valid JSON.", 
                    max_tokens=2048
                ))

                # ⚠️ 修复：使用 Claude 的响应结构：response.content[0].text
                response_text = response.content[0].text.strip() if response.content and response.content[0].type == 'text' else None
                
                if response_text is None:
                    raise ValueError("Claude topic generation returned empty content.")
                    
                # 简单的修复，例如去除可能存在的前后 markdown 代码块
                # 注意：由于 Claude 在 system prompt 引导下通常不使用 markdown，但保留清理步骤更安全。
...
# --- Display and Edit the generated “聊天话题池” (使用 Expander) ---
if st.session_state.chat_topics:
    st.write("---")
    with st.expander("📝 Your Chat Topic Pool (Editable)"):
        edited_topics = []
        for i, topic in enumerate(st.session_state.chat_topics):
            # 保持 st.text_input 的功能不变，只是把它移到 Expander 内部
...
- Length: Must vary between 15-35 words. (Updated to 15-35 for consistency)
- Model: claude-sonnet-4-5-20250929
...
**For Comments:**
- Uniqueness: Must be distinct.
- Length: Must vary between 5-25 words. (Updated to 5-25 for consistency)
...
**For Quotes:**
- Uniqueness: Must be distinct.
- Length: Must vary between 10-30 words. (Updated to 10-30 for consistency)
- Model: claude-haiku-4-5-20251001
...
if st.button("🚀 Generate Full Content Campaign", disabled=not st.session_state.get('chat_topics', []), type="primary"):
    ...
    with st.spinner("内容工厂启动，正在实时分析任务状态..."):
        ...
        # --- 【新增】预先提取 website_id 和 post_type_id ---
        # 注意：在上一轮修改中，orders 的 target_url 已经变成了 website_name 的 ID
        current_website_id = selected_item_details['orders']['target_url'] 
        
        # 获取 post_type_id (根据你的 get_order_items 查询结构)
        # 如果 post_type 是对象，取 ['id']；如果是直接的值，就直接用
        raw_post_type = selected_item_details.get('post_type')
        current_post_type_id = raw_post_type['id'] if isinstance(raw_post_type, dict) else raw_post_type
        # ----------------------------------------------------
        ...
        if existing_post_count < total_posts_required:
            ...
            for i in range(existing_post_count, total_posts_required):
                ...
                try:
                    ...
                    if post_meta_plan.get("k") and post_meta_plan["k"]:
                        kw = post_meta_plan['k'][0]
                        # 关键词优先级提升至首位，并使用强约束措辞
                        core_content_instruction = (
                            f"🚨 CRITICAL MENTION: You **MUST** integrate the project keyword '{kw}' naturally into your post, "
                            f"treating it as a PROPER NOUN, product, or specific system name. "
                            f"INTEGRATION METHOD (MANDATORY): Place it after a verb (e.g., 'I just tried the {kw} slot'). "
                            f"CRITICAL: The keyword MUST appear naturally in the middle of a sentence."
                        )
                    # 关键词任务不存在时，才检查 Brand/URL 任务
                    elif post_meta_plan.get("b") == 1 and brand_name:
                    ...
                    # --- 替换模型：使用 claude-sonnet-4-5-20250929 ---
                    post_text, post_cost = generate_text_from_ai_claude(claude_client, model="claude-sonnet-4-5-20250929", system_prompt=system_prompt_for_post, user_prompt="Go.", max_tokens=150, temperature=0.9)
                    ...
                    insert_res = supabase.table("content").insert({
                        "order_item_id": selected_order_item_id,
                        "task_type": "post",
                        "content_text": final_post_text,
                        "metadata": final_post_m,
                        "idempotency_key": post_idem_key,
                        "website_id": current_website_id,
                        "post_type_id": current_post_type_id
                    }).execute()
                    ...
                    for j in range(num_comments_per_post):
                        ...
                        # 2. 定义核心指令 (Comment 修正版)
                        if cm_meta.get("k") and cm_meta["k"] and isinstance(cm_meta["k"], list) and len(cm_meta["k"]) > 0: # ❗ 增加类型检查
                            kw = cm_meta['k'][0]
                            # 如果 Comment 被分配了关键词，使用强约束
                            core_content_instruction_comment = (
                                f"🚨 CRITICAL MENTION: You **MUST** naturally integrate the project keyword '{kw}' into your reply, "
                                f"treating it as a proper noun or specific system name. "
                                f"**Integration Method:** Place it after a verb (e.g., 'I just **tried** the {kw} game')."
                            )
                        else:
                            # 如果 Comment 未分配关键词，则保持排除 Brand/URL 的指令
                        ...
                        # 注意：Comment 的 Brand/URL 百分比通常设为 0%，所以这里主要处理关键词。
                        ...
                        # 动态生成黑名单指令
                        start_exclusion_instruction = ""
                        if st.session_state.used_start_phrases:
                            phrases = [f"'{p}'" for p in st.session_state.used_start_phrases]
                            # 告诉 AI 必须选择一个与这些开头完全不同的起始方式
                            start_exclusion_instruction = f"CRITICAL DIVERSITY: Do NOT start your comment with any of these recent three-word phrases: {', '.join(phrases)}. Choose a unique starting thought or word."

                        # 4. 生成内容
                        system_prompt_for_comment = ULTIMATE_SYSTEM_PROMPT.format(
                            context_block=f"""
                            You are replying to this post: "{final_post_text}".
                            {start_exclusion_instruction}
                            **DIVERSITY REMINDER**: Avoid common particles like 'Eh', 'Wah', 'So', 'Alamak','Bro', 'Oi', 'Nah' as the very first word. Start with a unique thought or word.
                            """,
                        ...
                        # --- 替换模型：使用 claude-haiku-4-5-20251001 ---
                        comment_text, comment_cost = generate_text_from_ai_claude(claude_client, model="claude-haiku-4-5-20251001", system_prompt=system_prompt_for_comment, user_prompt="Go.", max_tokens=80, temperature=0.95)
                        ...
                        if comment_text:
                            ...
                            interactions_to_insert.append({
                                ...
                                "task_type": "comment", 
                                "content_text": ctext, 
                                "parent_content_id": post_db_id, 
                                "metadata": final_cm_meta,
                                "idempotency_key": make_idempotency_key(selected_order_item_id, generation_phase_id, global_index, "comment", j),
                                "website_id": current_website_id,
                                "post_type_id": current_post_type_id
                            })
                    # C.2 引用生成
                    for j in range(num_quotes_per_post):
                        ...
                        # 2. 定义核心指令 (Quote 修正版) - 增强关键词和品牌/URL的优先级
                        ...
                        if qt_meta.get("k") and qt_meta["k"] and isinstance(qt_meta["k"], list) and len(qt_meta["k"]) > 0:
                            kw = qt_meta['k'][0]
                            # **使用与 Comment 相同的强约束措辞来强调关键词的集成**
                            core_content_instruction_quote = (
                                f"🚨 CRITICAL MENTION: You **MUST** naturally integrate the project keyword '{kw}' into your reply, "
                                f"treating it as a proper noun or specific system name. "
                                f"**Integration Method:** Place it after a verb (e.g., 'I just **saw** the {kw} game')."
                            )
                            # 确保 Brand/URL 在有关键词时被排除（如果它们不在qt_meta中被强制要求的话）
                        elif qt_meta.get("b") == 1 and brand_name:
                        ...
                        if st.session_state.used_start_phrases:
                            phrases = [f"'{p}'" for p in st.session_state.used_start_phrases]
                            start_exclusion_instruction = f"CRITICAL DIVERSITY: Do NOT start your quote with any of these recent three-word phrases: {', '.join(phrases)}. Choose a unique starting thought or word."

                        # 4. 生成内容
                        system_prompt_for_quote = ULTIMATE_SYSTEM_PROMPT.format(
                            context_block=f"""
                            You are quoting to this post: "{final_post_text}".
                            {start_exclusion_instruction}
                            **DIVERSITY REMINDER**: Avoid common particles like 'Eh', 'Wah', 'So', 'Alamak','Bro', 'Oi', 'Nah' as the very first word. Start with a unique thought or word.
                            """,
                            content_type="quote", 
                        ...
                        # --- 替换模型：使用 claude-haiku-4-5-20251001 ---
                        quote_text, quote_cost = generate_text_from_ai_claude(claude_client, model="claude-haiku-4-5-20251001", system_prompt=system_prompt_for_quote, user_prompt="Go.", max_tokens=100, temperature=0.9)
                        ...
                        if quote_text:
                            ...
                            interactions_to_insert.append({
                                ...
                                "content_text": qtext,
                                "parent_content_id": post_db_id,
                                "metadata": final_qt_meta,
                                "idempotency_key": make_idempotency_key(selected_order_item_id, generation_phase_id, global_index, "quote", j),
                                "website_id": current_website_id,
                                "post_type_id": current_post_type_id
                            })
                    # --- D. 互动内容批量入库 ---
            ...
            for index, post in enumerate(existing_posts):
                ...
                if comments_needed > 0:
                    for j in range(comments_needed):
                        ...
                        comment_pct_map = {'emoji': comment_emoji_percentage, 'hashtag': comment_hashtag_percentage}
                        comment_emoji_instruction, comment_hashtag_instruction = get_dynamic_instructions(comment_pct_map, cm_meta, brand_name, target_url, gambling_hashtags)
                        if cm_meta.get("k") and cm_meta["k"] and isinstance(cm_meta["k"], list) and len(cm_meta["k"]) > 0:
                            kw = cm_meta['k'][0]
                            # 如果 Comment 被分配了关键词，使用强约束
                            core_content_instruction_comment = (
                                f"🚨 CRITICAL MENTION: You **MUST** naturally integrate the project keyword '{kw}' into your reply, "
                        ...
                        if comment_text:
                            ...
                            interactions_to_insert.append({
                                ...
                                "parent_content_id": post_db_id, 
                                "metadata": final_cm_meta, 
                                "idempotency_key": make_idempotency_key(selected_order_item_id, generation_phase_id, post_db_id, "comment_fix", j),
                                "website_id": current_website_id,
                                "post_type_id": current_post_type_id
                            })
                # --- 补全引用 ---
                if quotes_needed > 0:
                    for j in range(num_quotes_per_post):
                        # ❗ 修复：使用 display_post_number
                        status_text.info(f"✍️ 正在为 Post #{display_post_number} 生成引用 {j+1}/{num_quotes_per_post}...")
                        ...
                        if quote_text:
                            ...
                            interactions_to_insert.append({
                                ...
                                "parent_content_id": post_db_id, 
                                "metadata": final_qt_meta, 
                                "idempotency_key": make_idempotency_key(selected_order_item_id, generation_phase_id, post_db_id, "quote_fix", j),
                                "website_id": current_website_id,
                                "post_type_id": current_post_type_id})
        ...
        if accumulated_cost > 0:
            ...
            try:
                ...
                update_response = supabase.table("order_items").update({
                    "cost": new_total_cost,
                    "status": "completed"  # 确保这里加上了状态更新
                }).eq("id", selected_order_item_id).execute()
